...
WS2812_RMT::WS2812_RMT(u16 n /*= 8*/, u8 pin_gpio /*= 2*/, u8 chn /*= 0*/, LED_TYPE t /*= TYPE_GRB*/)
{
	ledCounts = n;
	pin = pin_gpio;
	rmt_chn = chn;
...
void WS2812_RMT::setLedCount(u16 n)
{
	ledCounts = n;
	begin();
}

void WS2812_RMT::setLedType(LED_TYPE t)
{
	rOffset = (t >> 4) & 0x03;
	gOffset = (t >> 2) & 0x03;
	bOffset = t & 0x03;
...
void WS2812_RMT::setBrightness(u8 brightness)
{
	br = constrain(brightness, 0, 255);
}

esp_err_t WS2812_RMT::setLedColorData(int index, u32 rgb)
{
	return setLedColorData(index, rgb >> 16, rgb >> 8, rgb);
}

esp_err_t WS2812_RMT::setLedColorData(int index, u8 r, u8 g, u8 b)
{
	u8 p[3];
	p[rOffset] = r * br / 255;
	p[gOffset] = g * br / 255;
...
esp_err_t WS2812_RMT::set_pixel(int index, u8 r, u8 g, u8 b)
{
...
esp_err_t WS2812_RMT::setLedColor(int index, u32 rgb)
{
	return setLedColor(index, rgb >> 16, rgb >> 8, rgb);
}

esp_err_t WS2812_RMT::setLedColor(int index, u8 r, u8 g, u8 b)
{
	setLedColorData(index, r, g, b);
	return show();
}

esp_err_t WS2812_RMT::setAllLedsColorData(u32 rgb)
{
	for (int i = 0; i < ledCounts; i++)
	{
...
esp_err_t WS2812_RMT::setAllLedsColorData(u8 r, u8 g, u8 b)
{
	for (int i = 0; i < ledCounts; i++)
	{
...
esp_err_t WS2812_RMT::setAllLedsColor(u32 rgb)
{
...
esp_err_t WS2812_RMT::setAllLedsColor(u8 r, u8 g, u8 b)
{
...
esp_err_t WS2812_RMT::show()
{
	return rmtWrite(rmt_send, led_data, ledCounts*24);
}

uint32_t WS2812_RMT::Wheel(byte pos)
{
...
uint32_t WS2812_RMT::hsv2rgb(uint32_t h, uint32_t s, uint32_t v)
{
	u8 r, g, b;
	h %= 360; // h -> [0,360]
